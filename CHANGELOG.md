...
### 修复
- macOS 上关闭设置时间弹窗会导致整个 app 退出（最后一个窗口关闭触发默认终止）—— 在 RunEvent 里 `prevent_exit`，只有"退出"菜单显式调用才退出
- 提醒弹窗（午饭/下班/护眼）的"知道了"按钮和倒计时自动关闭都失效 —— 改走 `invoke('close_dialog')` 命令统一关窗
- 弹窗周围出现深色阴影 —— 去掉 `box-shadow` 和透明边距，卡片直接填满窗口
